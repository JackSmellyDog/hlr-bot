  ...
  public FileEntity write(FileEntity fileEntity, List<Hlr> hlrRowList) {
    try (XSSFWorkbook workbook = new XSSFWorkbook()) {
      ...
      for (Hlr hlr : hlrRowList) {
        ...
        createStringCell(row, 3,
            Optional.ofNullable(hlr.getPorted()).map(Ported::toString).orElse("-"));
        createStringCell(row, 4,
            Optional.ofNullable(hlr.getRoaming()).map(Roaming::toString).orElse("-"));
