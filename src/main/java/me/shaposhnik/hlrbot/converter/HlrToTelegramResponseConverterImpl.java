...
public class HlrToTelegramResponseConverterImpl implements HlrToTelegramResponseConverter {
  ...
  @PostConstruct
  void initMessages() {
    requestedPhoneNumberKey = bold(
        messageSource.getMessage("response.requested-phone-number", locale));
  ...
  public String convert(@NonNull Hlr hlr) {
    ...
    var mainBuilder = new StringBuilder()
        .append(requestedPhoneNumberKey).append(COLON_SPACE)
        .append(escapeSpecialSymbols(requestedPhoneNumber)).append(EOL)
        .append(providerIdKey).append(COLON_SPACE).append(hlr.getProviderId()).append(EOL)
        .append(msisdnKey).append(COLON_SPACE).append(escapeSpecialSymbols(hlr.getMsisdn()))
        .append(EOL)
        .append(networkKey).append(COLON_SPACE).append(escapeSpecialSymbols(hlr.getNetwork()))
        .append(EOL)
        ...
        .append(statusReceivedAtKey).append(COLON_SPACE).append(hlr.getStatusReceivedAt())
        .append(EOL);
    ...
    if (details != null && !details.isEmpty()) {
      ...
      details.forEach((k, v) ->
          detailsBuilder.append(TAB).append(bold(k)).append(COLON_SPACE)
              .append(escapeSpecialSymbols(v)).append(EOL));
    ...
    if (otherProperties != null && !otherProperties.isEmpty()) {
      ...
      otherProperties.forEach((k, v) ->
          otherPropertiesBuilder.append(TAB).append(bold(k)).append(COLON_SPACE)
              .append(escapeSpecialSymbols(v)).append(EOL));

      mainBuilder.append(otherPropertiesKey).append(COLON_SPACE).append(EOL)
  ...
  private String buildErrorMessage(String errorMessage, String requestedPhoneNumber) {
    return requestedPhoneNumberKey + COLON_SPACE + requestedPhoneNumber + EOL + errorKey
        + COLON_SPACE + errorMessage + EOL;
  }
